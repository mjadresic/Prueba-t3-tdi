...
export default async function handler(req, res) {
  ...
  const ordersFilesPattern = path.join(process.cwd(), 'public', 'orders_*.csv');
  ...
  try {
    ...
    const ordersFiles = fs.readdirSync(path.join(process.cwd(), 'public')).filter(file => file.startsWith('orders_'));
    for (const file of ordersFiles) {
      const filePath = path.join(process.cwd(), 'public', file);
      const data = fs.readFileSync(filePath, 'utf8');
      const yearMatch = file.match(/orders_(\d{4})/);
      if (yearMatch) {
        ...
        try {
          ...
          return res.status(500).json({ error: `Error parsing file ${file}: ${parseError.message}` });
        }
      }
    }
    ...
    fs.writeFileSync(path.join(process.cwd(), 'public', 'products_clean.json'), JSON.stringify(cleanProducts));
    fs.writeFileSync(path.join(process.cwd(), 'public', 'orders_clean.json'), JSON.stringify(cleanOrders));
